...
<mescroll-vue id="mescroll" class="mescroll-scroll" @init="initCallBack" :up="up" :down="down">
  <ul v-if="dataList.length !== 0">
    <li v-for="(data,index) in dataList" :key="index" @click="$router.push({name:'DomitoryDetail',params:{data:data,applyType:'2'}})">
      <div class="title">
        <span>审核状态:</span>
...
<script>
import { repairPage } from "@/service";
...
export default {
  ...
  methods: {
    ...
    async getList(page,status) {
        const {data} = await repairPage({  pageSize: page.size, pageIndex: page.num})
