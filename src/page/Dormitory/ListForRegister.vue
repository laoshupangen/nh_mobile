...
<mescroll-vue id="mescroll" class="mescroll-scroll" @init="initCallBack" :up="up" :down="down">
  <ul v-if="dataList.length !== 0">
    <li v-for="(data,index) in dataList" :key="index" @click="$router.push({name:'DomitoryDetail',params:{data:data,applyType:'3'}})">
      <div class="title">
        <span>审核状态:</span>
        <span class="status">审核中</span>
      </div>
      <div class="info">
        <span class="date">申请时间:{{data.parseTime}}</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getList(page,status) {
        let {data} = await checkinPage({ status, pageSize: page.size, pageIndex: page.num})
