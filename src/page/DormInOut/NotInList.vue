    ...
    <mescroll-vue id="mescroll" class="mescroll-scroll" @init="initCallBack" :up="up" :down="down">
      ...
      <ul v-if="dataList.length !== 0" class="noti-list">
        ...
        >
          <div class="title">
            {{ item.name }}
            <span style="color:#E18D43;">出宿舍</span>
          </div>
          <div>班级：{{ item.className }}</div>
          <div>宿舍号：{{ item.address }}</div>
          <div>出宿舍记录：{{ item.date }}</div>
          <div style="color:#E61E1E">未归时长：{{ item.count + '分钟'}}</div>
        </li>
      </ul>
    </mescroll-vue>
  </div>
...
import { GetNoReturnListByClass } from "@/service";
...
export default {
  ...
  methods: {
    initCallBack(mescroll) {
      this.model.startTime = this.$route.params.startTime ? this.$route.params.startTime:this.model.startTime;
      this.model.endTime = this.$route.params.endTime ? this.$route.params.endTime:this.model.endTime;
    ...
    async getList() {
      try {
        let data = await GetNoReturnListByClass({
