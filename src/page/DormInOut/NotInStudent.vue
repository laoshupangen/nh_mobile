    ...
    <mescroll-vue id="mescroll" class="mescroll-scroll" @init="initCallBack" :up="up" :down="down">
      ...
      <div :key="i" v-for="(item, i) in dataList">
        <ul v-if="dataList.length !== 0" class="noti-list">
          <li>
            <div>
              <span>{{'出宿舍记录：' + item.outTime}}</span>
              <span class="info-label">{{ '未归时长：' + item.count +'小时'}}</span>
            </div>
          </li>
        </ul>
      </div>
...
import { GetNoReturnList } from "@/service";
...
export default {
  ...
  methods: {
    ...
    async GetNoReturnList() {
      this.$vux.loading.show({
        text: "Loading"
      });
      try {
        let data = await GetNoReturnList({
